...
Spoken handoffs (September 12): switch to Odin / let me talk to Loki / use Loki's voice changes the active persona and its configured voice within the existing call. Handoffs bypass the language model. Finish-turn state records persona and response-preparation timings. Ordinary replies can return directly within a 6.5-second webhook budget; slower turns use signed polling. Fast audio uses a smaller phone-quality MP3. Production e551e2e3-edc8-4333-ae1c-821ef040abc0; rollback ec41f301-1a22-4073-abea-bbc1b37008ef. 133 unit tests passed. A consistent 1–2-second end-to-end response is not verified or guaranteed by this Gather/LLM/TTS architecture.

Live Loki test completed with two replies; readyMs was 1189 and 1744 ms (modelMs 718 and 1063). This is server preparation latency, not measured end-to-end caller latency.

Incoming calls: signed /phone-api/inbound accepts the configured owner caller number and rejects other numbers. Calls start as Thor and support spoken persona/voice handoffs and the same scoped tools. Incoming calls are available 24/7, independent of outbound pause/quiet hours and daily allowance, with up to 20 turns. Duplicate Twilio delivery reuses the call and greeting. /phone-api/inbound-status records completion. The setup-only setup-inbound endpoint backs up prior number routing to private KV and configures the verified existing Twilio number; no number purchase. Production 5745b8b0-2d74-4131-b934-f2582c108bb8. 135 tests passed; real incoming ringing needs an owner-originated call. Rollback requires restoring the number routing as well as Worker code. Caller number checking is not a biometric identity check.
