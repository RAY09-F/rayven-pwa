...
export function createPrismFoundry(host,{still=false,quality='balanced',onHover=()=>{},onSelect=()=>{},onStatus=()=>{}}={}) {
 const canvas=document.createElement('canvas');canvas.setAttribute('aria-label','Prism Foundry: drag to orbit, select a crystal to open its advisor');
 host.prepend(canvas);host.classList.add('prism-foundry');
 let renderer;
 ...
 catch(error){canvas.remove();host.classList.remove('prism-foundry');throw error;}
 renderer.setClearColor(0x050b12,0);renderer.setPixelRatio(Math.min(devicePixelRatio||1,1.5));renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.1;
 renderer.shadowMap.enabled=true;renderer.shadowMap.type=T.PCFSoftShadowMap;
 const scene=new T.Scene();scene.fog=new T.FogExp2(0x050b12,.045);
 ...
 const model=buildPrismFoundry(T);scene.add(model.root);model.update(0);
 scene.add(new T.HemisphereLight(0x4a7a9a,0x0a1a2a,.25));
 const key=new T.DirectionalLight(0xffffff,.77);key.position.set(4,7,5);key.castShadow=true;key.shadow.mapSize.set(1024,1024);key.shadow.radius=4;key.shadow.bias=-.0002;scene.add(key);
 const fill=new T.DirectionalLight(0xfff4e6,.175);fill.position.set(-5,3,-4);scene.add(fill);
 ...
 const studio=new T.Scene();studio.background=new T.Color(0x06101a);
 for(const [color,intensity,pos,w,h] of [[0xffe6c0,6,[4,6,3],5,3],[0x9ad4ff,3,[-6,3,-4],6,4],[0xfff0d0,1.5,[0,-3,6],8,2],[0x2a6a9a,1,[0,8,0],12,12]]){
 ...
 const labels=[{key:'core',name:'LOKI',role:'GOD OF MISCHIEF · THE ONE WHO MAKES',css:'#ffd84a',object:model.core},...model.stations.map(s=>({...s.a,object:s.gemGrp}))].map(info=>{
 ...
 function pick(){ray.setFromCamera(ptr,camera);return ray.intersectObjects(model.pickables,false)[0]?.object.userData.agentKey||null;}
 function positionLabels(){const scale=T.MathUtils.clamp(7/camera.position.distanceTo(controls.target),.7,1.4);
  for(const label of labels){label.object.getWorldPosition(world);direction.copy(world).sub(camera.position);const distance=direction.length();occRay.set(camera.position,direction.normalize());occRay.far=Math.max(0,distance-.3);const visible=!occRay.intersectObjects(label.key==='core'?model.occluders.filter(o=>o!==model.coreCrystal):model.occluders,false).length;
   projected.copy(world);projected.y+=label.key==='core'?1.45:-.3;projected.project(camera);
 ...
 function drawPreviews(){
  ...
  for(const [id,factory] of [['thor',createThor],['loki',null],['odin',createOdin]]){
   ...
   const miniature=factory?factory(T,{quality:'balanced',preview:true}):{root:model.root.clone(true),dispose(){this.root.removeFromParent();}};
 ...
 function frame(now){raf=0;if(disposed||contextLost||document.hidden)return;const dt=last?(now-last)/1000:0;last=now;controls.autoRotate=animated()&&!hasDragged;controls.enableDamping=animated();controls.update(dt);model.update(dt,{pointer:pointerInside?ptr:{x:0,y:0},hover,animated:animated()});scene.updateMatrixWorld(true);
  if(frames%2===0&&pointerInside&&!drag)hoverChanged(pick());positionLabels();drawPreviews();renderer.render(scene,camera);frames++;host.dataset.frames=String(frames);host.dataset.animated=String(animated());if(animated()||drag||settle-->0)schedule();
 }
 ...
 listen(canvas,'pointerdown',e=>{if(e.button!==0)return;pointer(e);drag={id:e.pointerId,x:e.clientX,y:e.clientY,moved:false};model.pulse();schedule();});
 listen(canvas,'pointerup',e=>{if(drag?.id!==e.pointerId)return;pointer(e);if(!drag.moved){const hit=pick();if(hit){selected=hit;onSelect(hit);}}drag=null;settle=animated()?12:1;schedule();});
 const cancel=()=>{drag=null;pointerInside=false;hoverChanged(null);settle=animated()?12:1;schedule();};
 listen(canvas,'pointercancel',cancel);listen(canvas,'pointerleave',()=>{pointerInside=false;hoverChanged(null);});listen(window,'blur',cancel);
 ...
 const buttons=host.parentElement.querySelector('.scene-controls'),arrange=buttons?.querySelector('[data-arrange]'),panel=buttons?.querySelector('.arrange-panel');const caption=host.parentElement.querySelector('.council-caption small'),oldCaption=caption?.textContent;if(caption)caption.textContent='Drag to orbit · click a crystal to inspect';const previousHidden=arrange?.hidden;if(arrange)arrange.hidden=true;if(panel)panel.hidden=true;
 ...
 const observer=new ResizeObserver(resize);observer.observe(host);resize();host.dataset.render='webgl';host.dataset.model='prism-foundry';onStatus('');
 document.fonts?.ready.then(()=>{if(!disposed)schedule();});
 return {select(id){selected=Object.keys(PRISM_AGENT_IDS).find(key=>PRISM_AGENT_IDS[key]===id)||id;model.pulse();settle=animated()?12:1;schedule();},setState(value){state=value;},setFocus(){},setStill(value){still=!!value;last=0;settle=1;schedule();},setQuality(value){quality=value;},resetView:resize,status(){return {ready:!disposed&&!contextLost,renderMode:contextLost?'fallback':'webgl',persona:'loki',scene:'prism-foundry',state,frames,animationTime:model.time,animated:animated(),hidden:document.hidden,rafActive:!!raf,width,height,threeRevision:T.REVISION,renderScale:renderer.getPixelRatio(),rendererResources:{...renderer.info.memory},advisors:model.stations.map(s=>PRISM_AGENT_IDS[s.a.key]),hover,selected,autoRotate:controls.autoRotate,hasDragged,cameraPosition:camera.position.toArray(),quality,shadowMapSize:key.shadow.mapSize.x,objects:labels.map(l=>{l.object.getWorldPosition(world);projected.copy(world).project(camera);return {id:l.key,x:(projected.x*.5+.5)*width,y:(-projected.y*.5+.5)*height};})};},dispose(){if(disposed)return;disposed=true;cancelAnimationFrame(raf);observer.disconnect();listeners.forEach(off=>off());controls.dispose();model.dispose();environment.dispose();key.shadow.dispose();ground.geometry.dispose();ground.material.dispose();renderer.dispose();renderer.forceContextLoss();canvas.remove();overlay.remove();host.classList.remove('prism-foundry');if(arrange)arrange.hidden=previousHidden;if(caption)caption.textContent=oldCaption;onHover(null);}};
}
