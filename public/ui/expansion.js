...
export function createExpansion({shell,prepare,parseField,openTools}){
 ...
 function detail(tool){const body=shell(tool.title,tool.description);navigation(body,'Local tools');const form=node('form','tool-form'),fields=[];
  ...
  form.addEventListener('submit',e=>{e.preventDefault();error.textContent='';output.replaceChildren();try{const args=Object.fromEntries(fields.map(([k,s,i])=>[k,parseField(i.value,s)]));const value=runLocalTool(tool,args);const completedAt=new Date(),printed=JSON.stringify(value,null,2);
    const receipt=node('div','action-receipt');receipt.append(node('span','receipt-mark','✓'),node('strong','','Calculation complete'));
    const time=node('time','',completedAt.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}));time.dateTime=completedAt.toISOString();receipt.append(time);
    output.append(receipt,node('p','tool-truth',tool.title+' · Computed in your browser from the inputs above.'));
    if(value&&typeof value==='object'&&!Array.isArray(value)){
      const table=node('dl','result-values');for(const [key,v] of Object.entries(value)){if(v===null||['string','number','boolean'].includes(typeof v)){table.append(node('dt','',key.replace(/([a-z])([A-Z])/g,'$1 $2').replaceAll('_',' ')),node('dd','',String(v)));}}if(table.children.length)output.append(table);
    }
    const details=node('details','result-data');details.append(node('summary','','Full result data'),node('pre','',printed));output.append(details);
    const actions=node('div','result-actions');
    actions.append(button('Save result',()=>{const record={tool:tool.id||tool.title,source:'ASGARD local workbench',completedAt:completedAt.toISOString(),inputs:args,result:value};const url=URL.createObjectURL(new Blob([JSON.stringify(record,null,2)],{type:'application/json'}));const a=node('a');a.href=url;a.download='asgard-'+(tool.id||'result')+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}),button('Prepare result for chat',()=>prepare(`Help me interpret this ${tool.title} result. These are user-supplied assumptions, not verified live figures.\nInputs: ${JSON.stringify(args)}\nResult: ${printed}`)));
    output.append(actions,node('p','tool-truth','Nothing was sent. Save a receipt or bring this result into your conversation.'));}catch(e){error.textContent=e.message;}});
