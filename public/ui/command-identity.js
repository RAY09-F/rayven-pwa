...
export const LOCK_MODES={thor:{palette:'locked',label:'CRIMSON / WHITE'},loki:{palette:'locked',label:'CRIMSON / WHITE'},odin:{palette:'locked',label:'CRIMSON / WHITE'}};
export const ROLES={
...
export function createCommandIdentity({figure}){
  ...
  function render(){const on=locked[current],role=ROLES[current];document.body.dataset.locked=String(on);document.body.dataset.identity=current;figure.palette(PALETTES[on?LOCK_MODES[current].palette:current]);figure.eyesGold(current==='thor'&&!on);$('identity-title').textContent=role.title;$('identity-description').textContent=role.description;$('identity-list').replaceChildren(...role.items.map((text,i)=>{const li=document.createElement('li'),n=document.createElement('span');n.textContent='0'+(i+1);li.append(n,document.createTextNode(text));return li;}));$('lock-in').textContent=on?'Stand down':'Lock in';$('lock-in').setAttribute('aria-pressed',String(on));$('identity-mode').textContent=on?'LOCKED IN · '+LOCK_MODES[current].label:'YOUR PERSONAL '+current.toUpperCase();$('lock-note').textContent=on?'Say “stand down” to restore your colors.':'Say “lock in” for '+LOCK_MODES[current].label.toLowerCase()+' focus.';}
  function set(id,value){const entering=value&&!locked[id];locked[id]=value;try{localStorage.setItem('asgard:lock-in:'+id,value?'1':'0');}catch{}if(id===current){render();if(entering)figure.lockTransition(true);else if(!value)figure.lockTransition(false);}}
  $('lock-in').addEventListener('click',()=>set(current,!locked[current]));render();
  return {select(id){figure.lockTransition(false);current=id;render();},command(text,id){const intent=lockIntent(text);if(intent!==null)set(id,intent);}};
}
