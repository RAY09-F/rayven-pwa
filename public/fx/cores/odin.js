...
(function () {
  ...
  function build(THREE) {
    ...
    const bladeG = new THREE.BoxGeometry(0.86, 0.26, 0.035); bladeG.translate(-0.43, 0, 0); geos.push(bladeG);   // hinged at its outer end
    O.blades = [];
  ...
  function applyIris(open) {
    ...
    const ang = 0.05 + (1 - open) * 1.15;
    for (let i = 0; i < BLADES; i++) O.blades[i].rotation.z = (i / BLADES) * TAU + Math.PI - ang;
  ...
  const CORE = {
    init(ctx) {
      ...
      try { O.env = makeEnv(THREE, ctx.renderer); if (O.env) ctx.scene.environment = O.env.texture; } catch (e) { console.warn('[odin core] environment map unavailable, lights only:', e && e.message); O.env = null; }
