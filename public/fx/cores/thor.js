...
(function () {
  ...
  function build(THREE) {
    ...
    if (C.presentation === 'focused') { O.shell.position.z = -0.35; O.hammer.position.z = 0.22; }
  ...
  const CORE = {
    init(ctx) {
      ...
      try { O.env = ctx.tier === 'svg' ? null : makeEnv(THREE, ctx.renderer); if (O.env) ctx.scene.environment = O.env.texture; } catch (e) { console.warn('[thor core] environment map unavailable, lights only:', e && e.message); O.env = null; }
      built = true;
