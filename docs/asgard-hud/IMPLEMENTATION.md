...
## Real-data-only correction rollout (in progress)

User rejected fake numbers and activity. The new adapter never renders reference-number fallbacks, example conversations, prewritten advisor claims, unmeasured telemetry/load, a made-up session/latency, or a false listening indicator. Real per-realm history is read from /history; calendar/timers, paper snapshots and timestamped events from /hud/summary. Errors clear stale data; unsupported plans/measurements are Unavailable. Chat and voice link to the preserved working /hall/ interface. Long history scrolls within the panel. Source-backed data is polled every 60 seconds while visible; this is not a streaming market feed. Paper trading remains simulated.

Preload version 9572fac7-3a3f-4203-b5fa-a7ea47513299 passed all 162 asset hashes and 11 successful samples over 319 seconds after an initial propagation mismatch restarted observation. Prior version ca4425cf-7e0a-40c6-a754-8f4c5f5ba8e7. Backend changes are limited to the read-only HUD adapter; production bindings and runtime were compared and unchanged. Root rollout and final verification still pending.

Real-data root rollout COMPLETE: bf8ac0d6-5ae6-49f6-8041-fb95e1a6276f at100%, floating-5129bddde7ea. Both canaries passed319seconds, both hosts passed162assetchecks, and unpinned live WebGL browser verification passed. See REAL-DATA-DEPLOYMENT.md.
