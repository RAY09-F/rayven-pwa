...
## Real data policy (supersedes the earlier copy-decision hold)

The user explicitly approved removing all fake readings, conversations and activity. Reference examples remain in the design config for traceability but the runtime adapter never displays them. All unavailable measurements are labeled Unavailable; unsupported counsel has an empty state. The HUD reads real per-persona history, shows actual summary request latency and its snapshot time, and marks the microphone off. Chat/voice opens the preserved full conversation interface. No generated response or human message was sent during verification. Recorded event timestamps and failures are displayed. Empty sources and failed sources are distinct, and failed refreshes remove stale readings. Market marks are recorded snapshots, not a streaming price feed. Calendar/timers/events refresh every 60 seconds while visible; there is no connected Thor plan tracker.

Validated locally with 171 passing repository tests and a real WebGL browser covering all three realms, data/history fixtures, failed refresh, lack of false voice indicators, and non-overflowing Unavailable tiles. See evidence/real-data-browser.js and real-data-loki.png. Production deployment is recorded separately after the Cloudflare canaries and unpinned verification finish.

Production rollout now verified at100%. See [real-data deployment receipt](REAL-DATA-DEPLOYMENT.md) for version IDs, both319-second canaries, both162-asset public-host checks and unpinned WebGL proof.
