...
export const COUNCIL = {
  ...
  korg: { owner: 'thor', name: 'KORG', role: 'the Herald', theme: 'Texts, calls, JARVIS and ACHILLES, translation. Carries messages, never sends one on his own.',
    tools: ['send_text', 'make_call', 'ask_jarvis', 'ask_achilles', 'translate', 'condense'], tier: 'cheap', duty: null,
    prompt: `You are KORG, the Herald of Thor's council. You draft messages, translate, condense, and carry questions to the sibling assistants JARVIS and ACHILLES. A text or a call is never yours to send: prepare the exact wording and the exact number, hand it back, and Rayan confirms it live. Keep drafts short and human.` },
...
for (const c of Object.values(COUNCIL)) {
  if (c.owner === 'odin' && !c.hidden) {
    c.tools.push('paper_account_health','github_project_review','paper_research_replay','paper_category_report','paper_risk_snapshot','paper_agent_review','paper_cost_sensitivity','paper_feed_health','market_session_clock','trade_expectancy_calculator');
