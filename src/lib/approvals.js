...
// Hard-confirm tools (SMS, calls) use this explicit approval as confirmation: approving
// one executes the exact stored action without asking again.
//
...
export async function createApproval(env, { persona, tool, input, tainted, sources, provenance, channel, councillor }) {
  ...
  const existing = list.find(a => a.status === 'pending' && Date.parse(a.expiresAt) > Date.now() && a.persona === persona && a.tool === tool && a.channel === (channel || 'web') && a.councillor === (councillor || null) && canonical(a.input) === canonical(input || {}));
  if (existing) return {ok:true,id:existing.id,record:existing,writes:0,reused:true};
...
export async function resolveApproval(env, id, decision, execute) {
  ...
  // Approval of the displayed recipient and payload IS the live confirmation.
  // Execute that stored action directly; never stage a second confirmation.
  if (HARD_CONFIRM_TOOLS.includes(rec.tool) && await checkPermission(env, rec.tool) === 'off') {
    return { ok: false, text: rec.tool + ' is turned off. Nothing was done.' };
  }
...
export const APPROVAL_TOOL_DEFINITIONS = [
  ...
  { name: 'approve', description: 'Approve a queued action by its four-digit number. Only when Rayan himself says so in this conversation. This is the single confirmation for the exact stored action, including texts and calls.', input_schema: { type: 'object', properties: { id: { type: 'string', description: 'the four-digit approval number' } }, required: ['id'] } },
