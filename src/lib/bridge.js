...
export async function getBridgeSnapshot(env,{now=Date.now(),since=0}={}) {
  const requests={
  ...
  const running=VISIBLE.map(id=>bridgePlan(values['status:'+id],id,now)).filter(Boolean);
  ...
  return {generatedAt:new Date(now).toISOString(),since:new Date(boundedSince).toISOString(),sources,
    needs:reviews.sort((a,b)=>timestamp(b.at)-timestamp(a.at)).slice(0,5),needsTotal:sources.approvals?reviews.length:null,
