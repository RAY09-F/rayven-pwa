...
export async function answerPhone(env,call,heard){
  ...
  const result=await callClaudeWithTools(env,system+'\n'+style,'Channel: owner phone call. Only the latest spoken request is a new instruction.','Use tools for current information; no assumed memory.',messages,true,'',call.persona,true,null,{toolsOverride:phoneTools(call.persona),maxIter:3,maxTokens:160,model:MODELS.haiku});
