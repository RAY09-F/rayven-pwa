...
export async function executionState(store, action, fields = {}, now = Date.now()) {
  ...
  if (action === 'read') return rows.map(({checkpoint,token,...row})=>({...row,
    state:row.routine?.status==='failed'?'failed':row.routine?.status==='unknown'?'unknown':row.state==='running'&&row.leaseUntil<=now || row.state==='waiting'&&row.question?.expiresAt<=now ? 'unknown' : row.state}));
  if (!['begin','step','heartbeat','finish','pause','claim','activateRoutineQuestion','routineComplete','routineProgress','resumeRoutineBatch'].includes(action)) throw Error('Invalid execution action');
  ...
  if (action === 'begin') {
    ...
    run.state='running';run.routine.status='running';run.at=now;run.leaseUntil=now+LEASE_MS;
    await store.put(KEY,rows);return true;
  } else if(action==='activateRoutineQuestion'||action==='routineComplete') {
    if(!run||run.token!==fields.token||!run.routine)return false;
    if(action==='activateRoutineQuestion'){
      if(run.state!=='waiting')return false;
      run.routine.ready=true;
    } else {
      if(!['done','failed','unknown','queued'].includes(fields.state))return false;
      run.routine.status=fields.state;run.state=fields.state;run.at=now;
      if(fields.state==='done')for(const step of run.steps)step.status='done';
    }
    await store.put(KEY,rows);return true;
  } else if (action === 'claim') {
    if(!run || run.state!=='waiting' || run.persona!==fields.persona || run.question?.revision!==fields.revision || run.question.expiresAt<=now||run.routine&&!run.routine.ready)return null;
    ...
    run.state='running';if(run.routine)run.routine.status='running';run.token=fields.token;run.at=now;run.leaseUntil=now+LEASE_MS;
  ...
  if(action==='routineProgress'){
    const step=run.steps[fields.index];
    if(!run.routine||!step||!['done','in_progress'].includes(fields.status))return false;
    if(fields.status==='in_progress'&&run.steps.some((s,i)=>i!==fields.index&&s.status==='in_progress'))return false;
    if(fields.status==='done'&&step.status!=='in_progress')return false;
    step.status=fields.status;
  }
  if (action === 'step' && !run.routine) {
  ...
  if (action === 'pause') {
    ...
    const parent=fields.checkpoint.options?.routineContinuation;
    if(parent){
      run.routine={id:parent.routineId,status:'waiting',ready:false};run.title=parent.title;
      run.steps=parent.plan.map((title,i)=>({id:String(i),title,status:i<parent.stepIndex?'done':i===parent.stepIndex?'in_progress':'pending'}));
    }
  ...
  if (action === 'finish') {
    ...
    run.state = fields.state==='done'&&run.routine?.status==='running'?'running':fields.state;
    // Failed/cancelled processing does not turn the active step into a success.
    if (fields.state === 'done'&&!run.routine) for (const s of run.steps) s.status='done';
  }
  run.at=now;run.leaseUntil=now+LEASE_MS;
  // Keep active work even with many completed runs; terminal receipts are bounded.
  const active=rows.filter(r=>r.routine&&['waiting','running','unknown','queued'].includes(r.routine.status) || r.state==='waiting' && r.question?.expiresAt>now || r.state==='running' && r.leaseUntil>now);
