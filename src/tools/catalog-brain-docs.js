...
const str=field;
export const TOOLS=[
 {...EXISTING.find(t=>t.name==='read_document'),name:'doc_to_markdown',description:'Convert a public document using the existing guarded Cloudflare toMarkdown reader. Disabled until conversion allowance is verified.',run:async(env,input)=>env.DOC_CONVERSION_ENABLED==='true'?EXISTING.find(t=>t.name==='read_document').run(env,input):'Document conversion is disabled until its allowance is verified. No request was made.',defer_loading:true,input_examples:[{url:'https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf'}]},
