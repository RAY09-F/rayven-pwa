...
test('lighting bridge requires pairing, validates states and rejects other origins',async()=>{
  const calls=[],token='a'.repeat(64),server=createCompanion({token,apply:async mode=>calls.push(mode),desktop:async()=>{}});
  ...
  try{
    ...
    const state=await (await fetch(base+'/status',{headers})).json();assert.equal(state.last.color,'#B00000');
