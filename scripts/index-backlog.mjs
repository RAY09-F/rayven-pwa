...
// A documentation-only commit must not change the released capability snapshot.
// Working files are still compared byte-for-byte to this revision below.
const head=proof?.sourceRevision??git('rev-parse','HEAD');
const remote=proof?.pushedRevision??git('rev-parse','origin/main');
...
function delivery(files){
 ...
 return {status:changed.length?'local only':head===remote?'pushed':'committed',revision:head,...(proof?{pushed_snapshot_revision:remote}:{remote_tracking_revision:remote}),local_changes:changed,note:proof?'Compared current relevant files to the validated released source/push snapshot; later documentation-only commits do not change this evidence.':'Compared relevant files to HEAD; origin/main is the locally observed ref, not a fresh remote query.'};
}
