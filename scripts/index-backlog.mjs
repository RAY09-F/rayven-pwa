...
const entries=starts.map((entry,i)=>{
 ...
 return {inventory_id:entry.inventory_id,family,title:entry.title,runtime_id:runtime,source:ref(masterPath,entry.start+1,end),source_fields:fields,
  ...
  repository_delivery:delivery(relevant),deployment:{status:'not checked',evidence:null},connection:{status:family==='L'?'not required':'unknown',provider:family==='L'?'None; local supplied-input computation':family==='B'?'See source dependency hints; credential/provider health not queried':fields['Recorded requirements']||fields['Existing capabilities to inspect']||'Depends on resolved workflow tools',dependency_hints:dependencyHints},
  verification:{status:family==='L'?'fixture pass':'untested',evidence:family==='L'?localTests:null,note:family==='L'?'All 36 run with representative input; targeted boundaries in same test file. No browser or live provider verification.':'Index/schema correspondence checks are not runtime verification.'},
  user_availability:{status:family==='B'?'permission restricted':'disabled',note:family==='B'?'Conservative audit status pending live policy/provider checks; not a claim that the server has disabled this tool.':family==='L'?'Not cleared for production availability by this audit; local implementation passes fixtures.':'Execution not established; proposal/preparation UI may be visible.'},
...
const report={format_version:1,evidence_date:'2026-09-08',scope:'485 retained entries; 445 capability catalogue entries plus 24 preparation briefs and 16 mission templates. No backend/live invocation.',source_sha256:createHash('sha256').update(master).digest('hex'),inventory_counts:counts,capability_count:445,total_indexed:entries.length,
 repository_revision:head,remote_tracking_revision:remote,verification:localTests,
...
const summaryRows=Object.keys(counts).map(f=>{const e=entries.filter(e=>e.family===f),status=e[0];return `| ${f} | ${e.length} | ${status.source_implementation.status} | ${status.verification.status} | ${status.connection.status} |`;}).join('\n');
...
for(const [file,content]of Object.entries(outputs)){
 if(process.argv.includes('--check'))assert.equal(read(file),content,`${file} is stale; run node scripts/index-backlog.mjs`);
