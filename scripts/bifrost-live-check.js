async page => {
 const base='https://asgrard-backend.rayanfahil2.workers.dev';const checks=[];
 ...
 check(await page.locator('.aperture').count()===1,'Live index serves Bifrost aperture');
 const release=await page.evaluate(async()=>await (await fetch('/ui/release.json',{cache:'no-store'})).json());check(release.name==='Bifrost Aperture','Live metadata identifies delivered build');
 ...
 for(const p of ['Thor','Loki','Odin']){await page.getByRole('button',{name:p,exact:true}).click();await page.waitForFunction(p=>AsgardUI.status().render.persona===p&&!AsgardUI.status().render.rafActive,p.toLowerCase());await page.screenshot({path:`output/playwright/live-${p.toLowerCase()}.png`});check(await page.locator('#presence-scene canvas').count()===1,`Live ${p} uses one renderer canvas`);}
 await page.setViewportSize({width:390,height:844});await page.getByRole('button',{name:'Loki',exact:true}).click();await page.screenshot({path:'output/playwright/live-loki-phone.png',fullPage:true});check(await page.evaluate(()=>document.documentElement.scrollWidth===innerWidth),'Live phone has no horizontal overflow');
 ...
 await page.screenshot({path:'output/playwright/live-chat.png'});await page.unroute(base+'/');
