...
await p.locator('.workspace-tools [data-open-arsenal]').click();await p.waitForFunction(()=>document.querySelectorAll('.tool-card').length>0);assert.ok(await p.locator('.tool-card').count()>10);await p.locator('.tool-card').first().click();assert.ok(await p.locator('.tool-form').count());await p.locator('.arsenal-close').click();results.push('Real catalog and tool forms');
...
assert.equal(await p.locator('.orbit-agent').count(),5);await p.locator('.orbit-agent').first().click();assert.ok((await p.locator('#arsenal-dialog').innerText()).includes('Prepare delegation'));await p.locator('.arsenal-close').click();
await p.locator('#orbital-settings').click();await p.locator('#still-motion').check();await p.getByRole('button',{name:'Done',exact:true}).click();results.push('Council and settings');
