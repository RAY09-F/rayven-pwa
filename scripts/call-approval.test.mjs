...
function fixture({expired=false,off=false,persona='thor'}={}) {
  const input={to:'+15555550123',message:'Hello, I am calling about a reservation.',purpose:'Ask about availability'};
  const data=new Map([
    ['approvals',JSON.stringify([{id:'1234',status:'pending',persona,tool:'make_call',input,description:'Call the displayed recipient about availability',expiresAt:new Date(Date.now()+(expired?-60000:60000)).toISOString()}])],
...
for (const persona of ['thor', 'loki', 'odin']) test(`${persona}: one explicit approval executes the exact call without staging a second yes`,async()=>{
  const {env,data,input}=fixture({persona}); const calls=[];
  ...
  assert.deepEqual(calls,[{tool:'make_call',args:input,persona}]);
  assert.equal(data.has(`pending:${persona}`),false);
